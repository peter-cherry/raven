...
export default function InteractiveDottedMap() {
  ...
  useEffect(() => {
    ...
    fetch('/map-dotted-pattern.svg')
      ...
      .then(svgContent => {
        ...
        // === Configure DOTS layer (top) - Light Theme ===
        ...
        if (dotsSvg) {
          ...
          // Update the style tag - light gray dots for light theme
          ...
          if (dotsStyleTag && dotsStyleTag.textContent) {
            dotsStyleTag.textContent = dotsStyleTag.textContent.replace(
              'fill: #fbb040;',
              'fill: #d4d4d8; transition: all 0.2s ease; cursor: pointer;'
            );
          }
          ...
          dots.forEach((dot) => {
            ...
            // Add hover listeners - blue hover for light theme
            circle.addEventListener('mouseenter', () => {
              circle.style.fill = '#2563eb'; // Primary blue on hover
              circle.style.transform = 'scale(1.5)';
              circle.style.transformOrigin = 'center';
            });

            circle.addEventListener('mouseleave', () => {
              circle.style.fill = '#d4d4d8'; // Light gray (matches --ds-border-strong)
              circle.style.transform = 'scale(1)';
            });
          });
        }

        // === Configure SMALL DOTS layer (under solid fill) - Light Theme ===
        ...
        if (smallDotsSvg) {
          ...
          // Update style tag - light gray dots for light theme
  ...
  return (
    <>
      ...
      {/* Solid fill layer - middle layer with subtle drop shadow for light theme */}
      <div
        style={{
          position: 'fixed',
          top: 0,
